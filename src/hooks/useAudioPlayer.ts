...
export const useAudioPlayer = (tracks: Track[]) => {
  ...
  const play = useCallback(async () => {
    if (audioRef.current && currentTrack) {
      console.log(
        `▶️ PLAY: Starting playback of "${currentTrack.title}" by ${currentTrack.artist}`,
      );
    ...
    if (isPlaying) {
      ...
      if (audioRef.current && currentTrack) {
        try {
          ...
          console.log(
            `📱 MOBILE: Direct play() failed, falling back to regular play function:`,
            error,
          );
  ...
  const next = useCallback(() => {
    ...
    console.log(
      `⏭️ NEXT: Moving from "${currentTrackTitle}" (${currentTrackIndex}) to "${nextTrackTitle}" (${nextIndex})`,
    );
  ...
  const handleEnded = useCallback(async () => {
    ...
    console.log(
      `🏁 TRACK ENDED: "${track?.title}" finished playing (repeat mode: ${currentRepeatMode})`,
    );
    console.log(
      `🏁 TRACK ENDED: wasPlayingRef: ${wasPlayingRef.current}, manuallyPaused: ${manuallyPausedRef.current}`,
    );
  ...
  const previous = useCallback(() => {
    ...
    console.log(
      `⏮️ PREVIOUS: Moving from "${currentTrackTitle}" (${currentTrackIndex}) to "${prevTrackTitle}" (${prevIndex})`,
    );
  ...
  const selectTrack = useCallback(
    (trackIndex: number) => {
      if (trackIndex >= 0 && trackIndex < tracks.length) {
        ...
        console.log(
          `🎵 SELECT: Changing from "${currentTrackTitle}" (${currentTrackIndex}) to "${selectedTrackTitle}" (${trackIndex})`,
        );
        ...
        manuallyPausedRef.current = false; // Clear manual pause flag when changing tracks
        console.log(
          `🎵 SELECT: Storing play state: ${wasPlayingRef.current ? "playing" : "paused"}`,
        );
        ...
        console.log(
          `⚠️ SELECT: Invalid track index ${trackIndex}, available tracks: ${tracks.length}`,
        );
      }
    },
