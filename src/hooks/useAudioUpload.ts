...
export const useAudioUpload = () => {
  ...
  const uploadAudio = useCallback(
    async (files: FileList): Promise<StoredTrack[]> => {
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        console.log(
          `🔄 Processing file ${i + 1}/${totalFiles}: ${file.name} (${Math.round(file.size / 1024)}KB)`,
        );
        ...
        try {
          ...
          console.log(
            `📊 Metadata extracted: ${metadata.title} by ${metadata.artist} (${metadata.duration}s)`,
          );
      ...
      console.log(
        `✅ useAudioUpload: Completed processing ${tracks.length}/${totalFiles} files successfully`,
      );
      return tracks;
    },
    [generateCoverArt],
